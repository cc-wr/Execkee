...
- **Phase 1 (multi-machine):**
  ...
  - ✅ **Daily plan with tentative guesses** (`daily-plan.json`): the task list **resets each local day** — completed items archive, incomplete confirmed ones carry forward, and Execkee **guesses today's tasks from the tracked files** (TRACKING.md + context-sources, once/day). Guesses are **tentative (LLM-generated)** until the user **approves via the primary** (`approve-task`/`reject-task`; approval promotes into `tasks.json`). A **"Tracked · no instance"** dashboard panel surfaces plan tasks with no managed instance (matched by name).
