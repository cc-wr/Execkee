...
- **Phase 1 (multi-machine):**
  ...
  - ✅ **Structured deferrals** (`deferrals.json`): the primary records a hold with `defer "<topic>" [--until YYYY-MM-DD]` (CLI/API), and the cycle suppresses that topic's related instance-surfaced (presumed) tasks + sentences from the dashboard. Relatedness is judged by an **LLM relatedness check** (by meaning — catches paraphrases, avoids coincidental word-share false positives), with a text match only as a fallback if the model is unavailable; active-by-date. `undefer`/`deferrals` manage them. Replaces relying on freeform TRACKING.md prose; brief v13 directs the primary to use `defer`.
