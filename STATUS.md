...
- **Phase 1 (multi-machine):**
  ...
  - ✅ **Structured deferrals** (`deferrals.json`): the primary records a hold with `defer "<topic>" [--until YYYY-MM-DD]` (CLI/API), and the cycle **deterministically suppresses** that topic's related instance-surfaced (presumed) tasks + sentences from the dashboard in BOTH the LLM and fallback paths (active-by-date; substring or all-significant-words match). `undefer`/`deferrals` manage them. Replaces relying on freeform TRACKING.md prose (which the model could miss); brief v13 directs the primary to use `defer`.
