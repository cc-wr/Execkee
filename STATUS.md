...
- **KI-6 — resume reverts to stale (adopt-time) conversation state (OPEN, high priority).** Reported 2026-06-16: the brain resuming the **primary surface** on restart keeps coming back to the same old state, not the latest conversation; **workhorse** instances have the identical problem (NOT fixed by KI-1, which only covered session-less *created* instances). Signature = the stored `sessionId` goes stale: the live transcript diverges from the id we `--resume`, so each resume reverts to a frozen snapshot. Leading hypothesis: interactive `claude --resume <id>` forks to a new id (headless `--resume` was verified to *continue* in place; interactive may differ). Fix direction: after each launch/resume, re-capture the LIVE session id (newest `.jsonl` in the instance's cwd slug, modified post-launch) and persist it, for both `launchPrimaryWindow` (currently captures once) and the workhorse launch path. Needs a definitive interactive fork-vs-continue test first.
